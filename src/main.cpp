...
void copyTest(time_t time1){
    auto hdd = new HardDisk(11700, time1, "Western Digital Caviar Blue 3.5 1TB 7200rpm 64MB SATA3 WD10EZEX",
                           7200);
     auto vga = new VideoCard(94000, time1, "GIGABYTE GeForce GTX 1660 Ti OC 6GB (GV-N166TOC-6GD) ", 6, 1800);
...
void moveTest(time_t time1){
   ...
   // hdds.insert(hdds.begin() + 1, HardDisk(1123, time1, "Western Digital  Green 123 222TB  128MB SATA3 ",
                            //               12000));
    HardDisk hdd;
    hdd = copyProduct<HardDisk>(hdds[1]);
}
